package auto.Day19.lianxi.service;
...
public class BusinessFlow {
    ...
    public static Response login_search_info(){
        //场景组合是由多个接口请求组装的
        //1、登录接口
        String loginData="{\"principal\":\"Test9350\",\"credentials\":\"123456\",\"appType\":3,\"loginType\":0}";
        Response loginRes = ApiCall.login(loginData);
        //提取token
        String token = loginRes.jsonPath().get("access_token");

        //保存到环境变量中
        Environment.saveEmmap("token",token);
        ...
        //保存到环境变量中
        Environment.saveEmmap("prodId",prodId);
        //3、商品信息接口
